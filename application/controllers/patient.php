...
class Patient extends CI_Controller
{
	...
	/******CONTACT DOCTOR LIST*****/
	function contact_doctor($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		
		$page_data['page_name']  = 'contact_doctor';
		$page_data['page_title'] = get_phrase('contact_doctor');
		$page_data['doctors']    = $this->db->get('doctor')->result_array();
	...
	/******RATE DOCTOR LIST*****/
	function rate_doctor($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		
		$page_data['page_name']  = 'rate_doctor';
		$page_data['page_title'] = get_phrase('rate_doctor');
		$page_data['doctors']    = $this->db->get('doctor')->result_array();
		
		$this->load->view('index', $page_data);
	}	
	
	function rate_doctor2($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		if (!empty($this->input->post())) {
			$this->db->delete('rating_patient', array('doctor_id' => $param1, 'patient_id' => $this->session->userdata('patient_id'))); 
			$this->db->insert('rating_patient', array('doctor_id' => $param1, 'patient_id' => $this->session->userdata('patient_id'), 'rating' => $this->input->post('rating'))); 
		}
		$page_data['page_name']  = 'rate_doctor2';
		$page_data['page_title'] = get_phrase('rate_doctor2');
		$page_data['doctors']    = $this->db->get('doctor')->result_array();
	...
	/******CONTACT DOCTOR2 LIST*****/
	function contact_doctor2($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		if (!empty($this->input->post())) {
		...
		$this->db->insert('message_patient', $insert_data); 
		}
		$page_data['page_name']  = 'contact_doctor2';
		$page_data['page_title'] = get_phrase('contact_doctor2');	
		$page_data['doctor_id'] = $param1;
		$page_data['messages'] = $this->db->get_where('message_patient', array(
			'doctor_id' => $param1,
			'patient_id' => $this->session->userdata('patient_id')
		))->result_array();
